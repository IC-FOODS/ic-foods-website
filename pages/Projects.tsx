            ...
            {rdResourcesLoading ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {rdResources.map((item, idx) => {
                  const getResourceTypeIcon = () => {
                    const type = item.resource_type?.toLowerCase() || '';
                    if (type.includes('guideline')) return <FileText size={20} />;
                    if (type.includes('report')) return <FileText size={20} />;
                    if (type.includes('portal')) return <Globe size={20} />;
                    if (type.includes('book')) return <BookOpen size={20} />;
                    if (type.includes('webinar')) return <Glasses size={20} />;
                    if (type.includes('training')) return <Settings size={20} />;
                    if (type.includes('template')) return <FileSpreadsheet size={20} />;
